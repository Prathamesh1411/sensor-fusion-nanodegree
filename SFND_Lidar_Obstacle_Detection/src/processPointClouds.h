...
public:
    ...
    std::vector<typename pcl::PointCloud<PointT>::Ptr> ClusteringOwnImplementation(const typename pcl::PointCloud<PointT>::Ptr &cloud, float clusterTol, int minSize, int maxSize);
    ...
    void clusterHelper3D(int indice, const std::vector<std::vector<float>>& points, std::vector<int> &cluster, std::vector<bool> &processed, KdTree* tree, float distanceTol);

    //USING COURSE INSTRUCTION
